...
def main():
    ...
    try:
        ...
        configfile = '/unified/roaming/extapps/vizalert/config/vizalerts.yaml'
    ...
    try:
        cleanup_dir('/unified/roaming/extapps/vizalert/sent_emails', 86400)
    except OSError as e:
        # Send mail to the admin informing them of the problem, but don't quit
        errormessage = 'OSError: Unable to cleanup sent_emails directory {}, error: {}'.format('/unified/roaming/extapps/vizalert/sent_emails', e)
        log.logger.error(errormessage)
        email_instance = emailaction.Email(config.configs['smtp.address.from'], config.configs['smtp.address.to'], config.configs['smtp.subject'], errormessage)
        emailaction.send_email(email_instance)
    except Exception as e:
        errormessage = 'Unable to cleanup sent_emails directory {}, error: {}'.format('/unified/roaming/extapps/vizalert/sent_emails', e)
        log.logger.error(errormessage)
        email_instance = emailaction.Email(config.configs['smtp.address.from'], config.configs['smtp.address.to'], config.configs['smtp.subject'], errormessage)
        emailaction.send_email(email_instance)
...
def get_alerts():
    ...
    try:
        ...
        for line in results:
            ...
            ### added by khoh chia jun 
            ### get time_interval parameter from csv file downloaded from tableau dashboard
            try:
                if 'time_interval' in line.keys():
                    default_setting = False
                    alert.time_interval = int(line['time_interval'])
                else:
    ...
    # modified by wong wai jian
    log.logger.info("Checking if the current time satisfies the custom time interval for executing the alert.")
    ...
    try:
            ...
            if not fileinput.isfirstline():
                ...
                for alert in alerts:
                    ...
                    if str(alert.subscription_id) == str(linedict['subscription_id']):
                        ...
                        ### edited by khoh chia jun         
                        if (not default_setting and datetime.datetime.now().minute % int(alert.time_interval) == 0 and not alert.is_test) or \
                           (default_setting and  (datetime.datetime.strptime(str(alert.run_next_at), "%Y-%m-%d %H:%M:%S") \
                           > datetime.datetime.strptime(str(linedict['run_next_at']), "%Y-%m-%d %H:%M:%S") \
                           and str(alert.schedule_id) == str(linedict['schedule_id'])) and not alert.is_test # test alerts are handled differently
                        ):

                                # For a test, run_next_at is anchored to the most recent comment, so use it as last run time
                                if alert.is_test:
                                    run_next_at_dt = datetime.datetime.strptime(linedict['run_next_at'], "%Y-%m-%d %H:%M:%S")
                                    run_next_at_log = (run_next_at_dt + datetime.timedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S")
                                    log.logger.debug(f"Alert {alert.subscription_id} is a test alert. Setting ran_last_at = run_next_at ({run_next_at_log})")
                                    alert.ran_last_at = alert.run_next_at
                                else:
                                    alert.ran_last_at = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
                                    ran_last_at_dt = datetime.datetime.strptime(linedict['ran_last_at'], "%Y-%m-%d %H:%M:%S")
                                    ran_last_at_log = (ran_last_at_dt + datetime.timedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S")
                                    log.logger.debug(f"Alert {alert.subscription_id} is scheduled. Setting ran_last_at {ran_last_at_log}")
                                ...
                                log.logger.debug(f"Alert {alert.subscription_id} last ran {seconds_since_last_run:.0f} seconds ago.")

                                execalerts.append(alert)
                        
                        if datetime.datetime.now().minute % int(alert.time_interval) == 0:
                            log.logger.debug(f"Current minute of {datetime.datetime.now()} is aligned with interval of {alert.time_interval} minutes.")
                        else:
                            log.logger.debug(f"Current minute of {datetime.datetime.now()} is NOT aligned with interval of {alert.time_interval} minutes.")
                        ###              
        ...
        with codecs.open(statefile, encoding='utf-8', mode='w') as fw:
            fw.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format("site_name", "subscription_id", "view_id",
                                                           ...
                                                           "schedule_id"))
            for alert in persistalerts:
                last_run_time = datetime.datetime.strptime(alert.ran_last_at, "%Y-%m-%d %H:%M:%S")
                next_run_time = last_run_time + datetime.timedelta(minutes=5)
                alert.run_next_at = next_run_time.strftime("%Y-%m-%d %H:%M:%S")
