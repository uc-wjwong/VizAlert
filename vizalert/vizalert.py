...
class VizAlert(object):
    ...
    def parse_action_fields(self):
        ...
        try:
            ...
            log.logger.debug(f"Finished matching fields. Now checking for action field errors for subscription_id {self.subscription_id}")
    ...
    def execute_alert(self):
        ...
        log.logger.info('Check PNG width {}'.format(self.viz_png_width))
        log.logger.info('Check PNG height {}'.format(self.viz_png_height))
        ...
        if self.trigger_data_rowcount == 0:
            ...
            def read_csv_as_set(filepath, exclude_columns):
                try:
                    with open(filepath, encoding='utf-8') as f:
                        reader = csv.DictReader(f)
                        return set(
                            tuple(sorted((k, v) for k, v in row.items() if k not in exclude_columns))
                            for row in reader
                        )
                except Exception as e:
                    log.logger.error(f"Error reading {filepath}: {e}")
                    return set()

            exclude_columns = [' Email Attachment', ' Email Subject', 'Details', 'Info']

            if len(kpi_files) >= 2:
                latest_file = kpi_files[0]
                second_latest_file = kpi_files[1]

                data1 = read_csv_as_set(latest_file, exclude_columns)
                data2 = read_csv_as_set(second_latest_file, exclude_columns)

                if data1 == data2:
                    log.logger.info(f"Duplicate alerts detected: the content of {os.path.basename(latest_file)} and {os.path.basename(second_latest_file)} are the same.")
                    log.logger.info("No alerts will be processed.")
                    return
            else:
                log.logger.info(f"Less than two KPIAlert files found (found {len(kpi_files)}), skipping duplicate alert check.")
            
            # # Proceed with processing alerts...
            # log.logger.debug("Proceeding with alert processing after duplicate check (or skipped check).")

            # # Compare the Last 2 files on last_tx_datetime
            #     def get_last_tx_datetime(filepath):
            #         try:
            #             with open(filepath, encoding='utf-8') as f:
            #                 reader = csv.DictReader(f)
            #                 for row in reader:
            #                     if 'last_tx_datetime' in row:
            #                         return row['last_tx_datetime']
            #         except Exception as e:
            #             log.logger.error(f"Error reading {filepath}: {e}")
            #         return None

            #     latest_val = get_last_tx_datetime(latest_file)
            #     second_latest_val = get_last_tx_datetime(second_latest_file)

            #     log.logger.info(f"Previous triggered data last_tx_datetime: {second_latest_val}")
            #     log.logger.info(f"Current triggered data last_tx_datetime:  {latest_val}")

            #     if latest_val is not None and latest_val == second_latest_val:
            #         log.logger.info(f"Duplicate alerts detected: {latest_val} "f"from files {os.path.basename(latest_file)} and {os.path.basename(second_latest_file)}. ")
            #         log.logger.info(f"last_tx_datetime is the same in both files. No alerts will be processed.")
            #         return
            
            log.logger.debug(f"Files differ. Now parsing fields")
            ...
            if self.trigger_data and len(self.error_list) == 0:
                ...
                log.logger.debug('Starting alert actions')
